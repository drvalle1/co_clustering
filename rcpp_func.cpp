...
// [[Rcpp::export]]
Rcpp::List getql(IntegerVector z, IntegerMatrix w, IntegerMatrix dat,
                 int ngrloc, int ngrspp, IntegerMatrix nmat) {
  IntegerMatrix nqly(ngrloc,ngrspp);
  IntegerMatrix nqlny(ngrloc,ngrspp);
  
  for(int i=0; i<dat.nrow();i++){
    for (int j=0; j<dat.ncol(); j++){
      nqly(z[i],w[j])=nqly(z[i],w[z[i],j])+dat(i,j);
      nqlny(z[i],w[j])=nqlny(z[i],w[z[i],j])+nmat(i,j)-dat(i,j);
    }
  }
  
  Rcpp::List resTemp = Rcpp::List::create(Rcpp::Named("nqly") = nqly,
                                          Rcpp::Named("nqlny") = nqlny);
  return(resTemp);
}
